...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--parent-key", default=None, help="Build a parent-id chain using this field name (e.g. parent_span_id).")
    p.add_argument("--session-key", default="session_id", help="Group flat events by this field (default: session_id).")
    p.add_argument("--max-depth", type=int, default=100, help="Cap tree depth (default: 100).")
    p.add_argument("--no-timing", action="store_true", help="Hide latency_ms columns.")
    p.add_argument("--show-args", action="store_true", help="Show inline args (or hang long ones below the node).")
    p.add_argument("--ascii", action="store_true", help="Use ASCII-only box characters (no unicode).")
